...
class Game:
    ...
    def __init__(
        self, gametype: str, players: list, dealer_i: int, auntie: int, card_color: bool
    ):
        ...
        if gametype == "Baseball":
            ...
            self.rules = (
                "1_card_screw: don't end up with the lowest card (Aces are low)"
            )
    ...
    def play(self):
        ...
        if self.dealtype == "sevencard":
            ...
            input(
                "click enter once people have determined if they're going low or high"
            )
            self.reveal_all_hands()
            self.winner_split()
        ...
        while highest_num_legs < 3:
            ...
            while True:
                ...
                for player_i in range(len(self.players)):
                    ...
                    player_in = input(
                        f"{player.name} are you in y/n? (You have {player.legs} legs)  "
                    )
                    if player_in == "y":
                        someones_in = True
                ...
                if someones_in == True:
                    player_list = (
                        self.players[player_in_index + 1 :]
                        + self.players[:player_in_index]
                    )
                    challenger_list = []
                    # side bets
                    for player in player_list:
                        player_challenging = input(
                            f"{player.name} are you in challenging {player_in_name} y/n? (they have {player.legs} legs)  "
                        )
                    ...
                    for challenger in challenger_list:
                        ...
                        winner = input(
                            f"who won {challenger.name} or {player_in_name}?  "
                        )
            ...
            if self.dealer_i > len(self.players) - 1:
                self.dealer_i = 0
            self.players = (
                players_original[self.dealer_i :] + players_original[: self.dealer_i]
            )
    ...
    def one_card_screw(self):
        ...
        up_or_down = input("Are we passing down or up D/U?  ")
        stack_num = input(
            "How many stacks of 5 chips are each person going to start with?  "
        )
        ...
        while len(self.players) != 1:
            ...
            for player_i in range(len(self.players)):
                player = self.players[player_i]
                player_in = input(
                    f"{player.name} are you passing y/n? (You have {player.legs} stacks left)  "
                )
            ...
            while losser_recorded == False:
                try:
                    losser = input(
                        "Who had the lowest card? (Enter player name), if no losser enter 'none'   "
                    )
                    for player in self.players:
    ...
    def d_and_g(self):
        ...
        up_value = float(
            input("How much do you want to charge for the last card to come up?  ")
        )
        while self.game_over == False:
            # deal two down cards to each player
            self.deal_down(display=False)
            ...
            for player in self.players:
                card_type = input(
                    f"{player.name} do you want your last card up for {up_value} (y/n)?   "
                )
                if card_type == "y":
                    card = self.deck.draw_card()
    ...
    def kings(self):
        ...
        while self.game_over == False:
            ...
            for player_tuple in players_in:
                ...
                result = input(
                    f" {player_tuple[0].name} did you win, lose, or tie (w/l/t)?  "
                )
            ...
            # reset players hands, deck, and table hand, rotate deal
                ...
                if self.dealer_i > len(self.players) - 1:
                    self.dealer_i = 0
                self.players = (
                    players_original[self.dealer_i :]
                    + players_original[: self.dealer_i]
                )
    ...
    def seven_twentyseven(self):
        ...
        while self.game_over == False:
            ...
            for player in self.players:
                player.legs = 0
                up = int(
                    input(f"{player.name} which card do you want flipped (1 or 2)?  ")
                )
                up -= 1
            ...
            while True:
                ...
                for player in eligible_players:
                    ...
                    hit = input(
                        f"{player.name} (you have {3 - player.legs} passes left) do you want another card y/n?  "
                    )
                    if hit == "y":
                        card = self.deck.draw_card()
                        player.hand.add_up_card(card)
    ...
    def passing_cards(self, pass_direction, num_cards):
        ...
        for player in self.players:
            ...
            while pass_recorded == False:
                cards_to_pass = input(
                    f"{player.name} what {num_cards} cards to you want to pass (cards are ordered 1-7 left to right) (input indexes as A,B,...)?  "
                )
                try:
    ...
    def discarding_cards(self, num_cards):
        for player in self.players:
            ...
            while discard_recorded == False:
                cards_to_discard = input(
                    f"{player.name} what {num_cards} cards to you want to discard (cards are ordered 1-7 left to right) (input indexes as A,B,...)?  "
                )
                try:
    ...
    def buy_down_four(self, four_price):
        ...
        for player in self.players:
            ...
            while four_recorded == False:
                try:
                    four_bought = input(
                        f"{player.name} are you buying a down four for {four_price} y/n?  "
                    )
                    ...
                    if four_bought == "y":
                        ...
                        if four_found == False:
                            print(
                                "Error processing buying of four. System shows you do not have a four."
                            )
        ...
        for player in self.players:
            if player.hand.cards[-1].rank == "4":
                ...
                while four_recorded == False:
                    try:
                        four_bought = input(
                            f"{player.name} are you buying a up four for {four_price} y/n?  "
                        )
    ...
    def players_reorder_hands(self, num_cards):
        for player in self.players:
            ...
            while reorder_recorded == False:
                try:
                    order_list = input(
                        f"{player.name} what order do you want your hand A,B,C...?  "
                    )
    ...
    def winner(self):
        ...
        while winner_recorded == False:
            try:
                winners = input(
                    "Who is the winner? (Enter player name, if tie seperate names by ',')   "
                )
    ...
    def winner_split(self):
        ...
        while winner_recorded == False:
            try:
                print(
                    "special inputs: if only high winer enter 'winner:', if multiple high winners enter 'winner_1,winner_2:winner_3'"
                )
                winners = input(
                    "Who is the winner? (Enter winners names in the form high_winners:low_winners)  "
                )
    ...
    def bet(self, player, current_bet: int):
        ...
        while bet_recorded == False:
            try:
                bet_action = input(
                    f"{player.name} what is action (fold, check, call, raise)? current bet is {current_bet} you have {player.bet} in:  "
                )
